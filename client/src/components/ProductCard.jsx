...
function ProductCard({ product, onAddToCart, onCardClick, disableLink, productIndex, inCartQty = 0 }) {
  ...
  return (
    <div
      className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col group border border-gray-200 dark:border-gray-700 h-[420px] min-h-[420px] max-h-[420px] cursor-pointer"
      onClick={onCardClick}
      tabIndex={0}
      role="button"
    ...
    >
      {!imageError && mainImage ? (
        <img
          ...
          alt={product.name}
          className="w-full h-48 object-cover transition-transform duration-300 group-hover:scale-105 mb-4 rounded-lg"
  ...
  {/* махаме описанието под снимката */}
      <div className="flex flex-row items-center justify-between mt-2 gap-2 w-full">
        <button
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          style={{ minWidth: '80px' }}
